          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {menuItems.map((item) => (
              item.href && item.href.startsWith('/') ? (
                ...
                >
                  <NavLink to={item.href} className={({isActive}) => `font-medium text-sm lg:text-base transition-colors duration-300 ${isActive ? 'text-primary-600' : 'text-gray-700 hover:text-primary-600'}`}>{item.label}</NavLink>
                </motion.div>
              ) : (
                <motion.a
                  key={item.label}
          ...
          {isOpen && (
            ...
            >
              {menuItems.map((item, idx) => (
                item.href && item.href.startsWith('/') ? (
                  <motion.div key={item.label} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04, type: 'spring', stiffness: 220, damping: 24 }}>
                    <NavLink to={item.href} onClick={() => setIsOpen(false)} className={({isActive}) => `block py-2.5 font-medium ${isActive ? 'text-primary-600' : 'text-gray-700 hover:text-primary-600'}`}>{item.label}</NavLink>
                  </motion.div>
                ) : (
                  <motion.a
                    key={item.label}
